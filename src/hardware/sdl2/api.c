...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
	...
	for (dev_i = 0; dev_i < dev_count; ++dev_i) {
		...
		//printf("Found audio device %d: %s\n", i, devc->sdl_device_name);
...
int sdl_data_callback(int fd, int revents, void *cb_data)
{
	...
	//sr_err("Remaining samples to be acquired: %lu", devc->limit_samples_remaining);
	...
	//sr_err("SDL2 Format: %lu", devc->sdl_device_spec.format);
	...
	//sr_err("SDL2 received %lu bytes (%lu samples)!", recv_bytes, SDL_BYTES_TO_SAMPLES(recv_bytes, devc->sdl_device_spec));
...
static int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
	...
	//devc->sdl_device_spec.format = AUDIO_S32; //Only for testing!!! Usualy is already autodetected.
	...
	//sr_err("Open SDL2 device for capture: %d", devc->sdl_device_handle);
